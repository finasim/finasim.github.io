...
function BadgeCard({ name, subtitle, badgeImage, note, progress, delay }: BadgeCardProps) {
  ...
  return (
    <motion.div
      ...
      className="relative flex flex-col items-center gap-3 rounded-xl border border-slate-200 bg-white p-5 text-center dark:border-slate-800 dark:bg-surface-card"
    >
      <div
      ...
      >
        {badgeImage ? (
          <img src={badgeImage} alt={`${name} badge`} className="h-24 w-24 object-contain" />
        ) : (
      ...
      {inProgress && (
        <div
          className="absolute left-1/2 top-5 h-24 w-24 -translate-x-1/2 flex items-center justify-center"
          role="img"
